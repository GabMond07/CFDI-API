...
async def get_user(rfc: str):
    start_time = datetime.now(timezone.utc)
    user = await db.user.find_unique(where={"rfc": rfc}, include={"role": True})
...
async def create_access_token(data: dict, expires_delta: timedelta = None):
    ...
    # Incluir permisos del rol en el token
    role = await db.roles.find_unique(where={"id": to_encode["role_id"]})
    to_encode["scopes"] = role.permissions.get("scopes", []) if role and role.permissions else []
...
async def create_refresh_token(data: dict, expires_delta: timedelta = None):
    ...
    # Incluir permisos del rol en el refresh token
    role = await db.roles.find_unique(where={"id": to_encode["role_id"]})
    to_encode["scopes"] = role.permissions.get("scopes", []) if role and role.permissions else []

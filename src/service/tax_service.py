...
async def consultar_taxes(filtros: FiltroTax, user_rfc: str):
    ...
    # Obtener Concept_IDs que pertenecen a esos CFDIs
    conceptos = await db.concept.find_many(
        where={"CFDI_ID": {"in": cfdi_ids}}
    )
    ...
    ordenar_por = filtros.ordenar_por if filtros.ordenar_por in ["Tax_ID", "Type", "Tax", "Rate", "Amount"] else "Tax_ID"
